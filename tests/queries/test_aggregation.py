...
class TestAggregation:
    def setup_method(self):
        ...
        Product(title="", cost=0.0, quantity=0, product_type="", config={})
        Product.Q.drop_collection(force=True)

        # Initialize the ProductImage class
        product_id_value = str(ObjectId())
        ProductImage(url="", product_id=product_id_value)
    ...
    @pytest.mark.asyncio
    async def test_async_aggregation_math_operation(self):
        data = [
            self.Product(
                title=str(i),
                cost=float(i),
        ...
        max_ = await self.Product.AQ.simple_aggregate(aggregation=Max('cost'))
        assert max_ == {'cost__max': 4}

        min_ = await self.Product.AQ.simple_aggregate(aggregation=Min('cost'))
        assert min_ == {'cost__min': 1}

        sum_ = await self.Product.AQ.simple_aggregate(aggregation=Sum('cost'))
        assert sum_ == {'cost__sum': 10}

        avg_ = await self.Product.AQ.simple_aggregate(aggregation=Avg('cost'))
        assert avg_ == {'cost__avg': 2.5}

        simple_avg = await self.Product.AQ.aggregate_sum('cost')
        assert simple_avg == 10.0

        simple_max = await self.Product.AQ.aggregate_max('cost')
        assert simple_max == 4

        simple_min = await self.Product.AQ.aggregate_min('cost')
        assert simple_min == 1

        simple_avg = await self.Product.AQ.aggregate_avg('cost')
        assert simple_avg == 2.5
